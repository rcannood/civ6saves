...
plot_terrain_map <- function(tib, alpha = 1) {
  ...
  plot_empty_map(tib) +
    ggforce::geom_regon(aes(fill = terrain_name), data = tib %>% filter((feature_name %|% "") != "Ice"), alpha = alpha) +
    ggforce::geom_regon(aes(fill = feature_name), data = tib %>% filter(feature_name == "Ice"), alpha = alpha) +
